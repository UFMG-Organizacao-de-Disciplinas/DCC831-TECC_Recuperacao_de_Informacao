...
Frontier as a list of URLs to be crawled could have many advantages, such as allowing for a queue of URLs to be crawled. But, it would also allow the existance of a list of many duplicates.

To avoid this, we could use a set to store the URLs that have already been crawled. This would allow us to avoid crawling the same URL multiple times, but now we lose the ability to crawl URLs in a queue fashion, since we could only \textbf{pop} the top URL from the set. This would be a problem, since, if the top URL is the same as the last one... I guess...

I could also go for a deque, since I could pop from both ends, but it goes down the same path as the list. So...

I'll just go back a few steps at the crawling process. Since the sitemaps are so more clean to process than the usual HTML pages, I will just grab the main outlink domains and make it spread further.

\subsubsection{Revisiting}

Remove already visited or just shoo them away?

% \section{Introduction}


| Description | Threads | Collected URLs | Frontier | Time       | Mean Time per URL | Expected Time for 100k Collected URLs |
| ----------- | ------- | -------------- | -------- | ---------- | ----------------- | ------------------------------------- |
| Mono thread | 1       | 1647           | 5767     | 31min44seg | X                 | X                                     |
| Mono thread | 1       | 1000           | 3000     | 14min19    | X                 | X                                     |
| Thread Pool | 10      | 1002           | 3268     | 9min44     | X                 | X                                     |
| Threading   | 1       | 1000           | 3000     | 14min19    | X                 | X                                     |
